...
from schemas import (
    ...
    PromptCreate, PromptResponse, PromptType
)
from auth import (
...
app = FastAPI(
    title="Prompt API",
    description="API for managing game prompts with user authentication and versioning",
    version="1.0.0"
)
...
def get_prompt(
    ...
    if user_prompt:
        ...
        return PromptResponse.from_orm(user_prompt)
    ...
    if default_prompt:
        ...
        return PromptResponse.from_orm(default_prompt)
    
    raise HTTPException(
        status_code=404,
        detail=f"No prompt found for game {game_id} and type {prompt_type}"
    )
...
def create_prompt(
    ...
    logger.info(f"New prompt created: user {current_user.username}, game {prompt.game_id}, type {prompt.prompt_type}, version {new_version}")
